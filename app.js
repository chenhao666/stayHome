...
App({
  ...
  onShow: function (options) {
    ...
    //登录环信
    if (wx.getStorageSync('myUsername')){
      var options = {
        apiUrl: WebIM.config.apiURL,
        user: wx.getStorageSync('myUsername'),
        pwd: wx.getStorageSync('myUserPsw'),
        grant_type: 'password',
  ...
  globalData: {
    ...
    resUrl: 'http://192.168.1.62:8080/ourHouse/',
  ...
  //城市列表的分类
  formatList:function(arr,keyword){
    // 对arr进行排序
    let Arr = []
    let Arr1 = []
    let Arr2 = []
    for(let i=0;i<arr.length;i++){
      Arr.push(arr[i].cityPin)
    }
    Arr1 = Arr.sort();
    for (let i = 0; i < Arr1.length; i++){
      for (let j = 0; j < arr.length; j++){
        if (Arr1[i] == arr[j].cityPin){
          Arr2.push(arr[j])
        }
      }
    }
    ...
    let reg = /\b(\w)|\s(\w)/g;
    let k = 0;
    let firstWord = Arr2[0][keyword].charAt(0).toUpperCase();//获取第一个分类字母
    Arr2.map((v) => {
