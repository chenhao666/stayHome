Page({
  ...
  onLoad: function (options) {
    ...
    if (options){
      if (options.enterType){
        this.setData({
          enterType: JSON.parse(options.enterType)
        })
      }
  ...
  onShow: function () {
   //this.onLoad(this.options)
    let that = this;
    if (this.options.optionsData) {
      let bb = JSON.parse(this.options.optionsData);
      that.setData({
        data: bb,
        jumpStatus: true
      })
 ...
 */
  ...
  updateBtn(e){
    ...
    if(this.data.source == 0){
      ...
      wx.navigateTo({
        //传一个index，用于重新调取接口，获取地址信息
        url: "/pages/order/updataGoodAddress/updataGoodAddress?indexValue=" + JSON.stringify(that.data.objData)
      })
    }
  ...
  addAddress: function(){
    let that = this;
    //判断要跳转到好物添加地址还是正常的添加地址
    if(that.data.source == 1){
      wx.navigateTo({
        url: '/pages/order/addGoodAddress/addGoodAddress?pageStu=' + that.data.pageStu
      })
    }else{
      wx.navigateTo({
    ...
    if (this.data.enterType == 2){
      ...
      if (that.data.source == 0) {
        if (that.data.jumpStatus) {
          ...
          console.log(aa)
          wx.redirectTo({
            url: '/pages/order/confirm/confirm?obj=' + objData + '&list=' + aa,
          })
        }
        ...
        console.log(aa)
        wx.redirectTo({
          url: '/pages/order/goodConfirm/goodConfirm?obj=' + objData + '&list=' + aa,
        })
      }
    }
  }
