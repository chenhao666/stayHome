...
Page({
  ...
  onShow: function () {
    ...
    let that = this;
    if (this.options.indexValue) {
      // 获取带过来的数据，显示在修改地址页面
      that.setData({
        indexValue: JSON.parse(this.options.indexValue),
        requireData: {
          linkman: JSON.parse(this.options.indexValue)[2],
          mobileNum: JSON.parse(this.options.indexValue)[3],
          province: JSON.parse(this.options.indexValue)[4],
          provinceId: JSON.parse(this.options.indexValue)[5],
          city: JSON.parse(this.options.indexValue)[6],
          cityId: JSON.parse(this.options.indexValue)[7],
          distincts: JSON.parse(this.options.indexValue)[8],
          distinctId: JSON.parse(this.options.indexValue)[9],
          address: JSON.parse(this.options.indexValue)[10]
        }
      })

    } else if (this.options.pageStu) {
      let that = this;
      that.setData({
        pageStu: this.options.pageStu
      })
    }
  },
