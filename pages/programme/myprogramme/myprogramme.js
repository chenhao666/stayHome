...
Page({
  ...
  programList: function(e){
    ...
    wx.navigateTo({
      url: '/pages/order/productList/productList?designId=' + this.data.requireData[e.currentTarget.dataset.index].designId + '&designType=1'
    })
  },
  getData: function(e){
    let that = this;
    ...
    getApp().ajax("queryUserProgram", { token: checkKey }, 'post', function (res) {
      if (!res.data.userPrograms || res.data.userPrograms.length == 0) {
        // 控制无信息时的状态
          that.data.notMessage = true;
      } else {
        that.data.notMessage = false;
        for (let i = 0; i < res.data.userPrograms.length; i++) {
          res.data.userPrograms[i].sendTime = res.data.userPrograms[i].sendTime.split('.')[0];
        }
        that.setData({
